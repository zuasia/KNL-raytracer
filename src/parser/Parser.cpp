...
void Parser::parseTrimesh(Scene* scene, TransformNode* transform, const Material& mat)
{
  ...
  for( ;; )
  {
    ...
    switch( t->kind() )
    {
      ...
      case MAPPOINTS:
        _tokenizer.Read( MAPPOINTS );
        _tokenizer.Read( EQUALS );
        _tokenizer.Read( LPAREN );
        if( RPAREN != _tokenizer.Peek()->kind() )
        {
          tmesh->addVertexMap( parseVec2d() );
          for( ;; )
          {
             const Token* nextToken = _tokenizer.Peek();
             if( RPAREN == nextToken->kind() )
...
void Parser::parseAreaLight( Scene* scene )
{
  Vec3d position;
  Vec3d color;
  ...
  bool hasPosition( false ), hasColor( false ), hasArea( false );
  ...
  for( ;; )
  {
     ...
     switch( t->kind() )
     {
       ...
       case AREA:
	 if( hasArea )
            throw SyntaxErrorException( "Repeated 'area' attribute", _tokenizer );
         area = parseScalarExpression();
         hasArea = true;
         break;
       ...
       case RBRACE:
		 {
			 ...
			 if(area_check) {
				 float delta;
				 delta = area/sample_count;
				 int i, j;
				 for (int i=0; i<sample_count; ++i) {
					 for (int j=0; j<sample_count; ++j) {
						 point_position[0] = position[0]+delta*(i-sample_count/2);
						 point_position[1] = position[1];
						 point_position[2] = position[2]+delta*(j-sample_count/2);

						 scene->add( new PointLight( scene, point_position, color/(float) (sample_count*sample_count), constantAttenuationCoefficient, 
									 linearAttenuationCoefficient, quadraticAttenuationCoefficient ) );
					 }
				 }
			 }
			 else {
				 point_position[0] = position[0];
				 point_position[1] = position[1];
				 point_position[2] = position[2];
				 scene->add( new PointLight( scene, point_position, color, constantAttenuationCoefficient, 
							 linearAttenuationCoefficient, quadraticAttenuationCoefficient ) );
			 }
